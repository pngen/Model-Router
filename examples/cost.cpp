// Model Router example - cost-aware routing.
// Copyright 2026 Summon Software Labs.
// Licensed under the Apache License, Version 2.0.
//
// SPDX-License-Identifier: Apache-2.0
//
// Two backends both serve the capable model but publish different current
// prices. Cost is a ranking factor, never a hard constraint, so the cheaper
// current legal route wins and both estimates are printed. The example never
// prompts, never waits, and always exits 0 on success.
...
int run() {
  ...
  publish_costs(router, catalog, BackendId(1), model_router::PriceGeneration(1), 500, 700, 900,
                now);
  publish_costs(router, catalog, BackendId(2), model_router::PriceGeneration(1), 300, 400, 100,
                now);
  ...
  std::cout << "cost example: two capable backends publish different current prices\n";
  for (const model_router::RankedCandidate& candidate : outcome.route.ranking) {
    const model_router::FactorValue* cost =
        candidate.find(model_router::RankingFactor::COST_TOTAL);
    require(cost != nullptr, "cost factor must be present");
    std::cout << "candidate rank=" << candidate.rank
              << " model_id=" << candidate.key.model_id.value()
              << " backend_id=" << candidate.key.backend_id.value()
              << " estimated_total_micros=" << cost->raw
              << " cost_known=" << (cost->known ? 1 : 0)
              << " score=" << candidate.score << "\n";
  }
  // The winner's current price evidence is read from the winning backend's
  // binding, so the printed estimate is the exact evidence that was priced.
  model_router::BackendDescriptor winner_backend;
  require(router.find_backend(outcome.decision.authority.backend_id, &winner_backend),
          "find_backend");
  const model_router::ModelBinding* winner_binding =
      winner_backend.find_binding(outcome.decision.authority.model_id);
  require(winner_binding != nullptr, "winner model binding");
  std::cout << "cost_unit=" << winner_binding->cost.unit.currency << "/"
            << winner_binding->cost.unit.basis << " knownness="
            << model_router::to_string(winner_binding->cost.knownness)
            << " price_generation=" << winner_binding->cost.price_generation.value() << "\n";
  std::cout << "winner_model_id=" << outcome.decision.authority.model_id.value()
            ...
            << " winner_estimated_total_micros="
            << winner_binding->cost.estimated_total_micros << "\n";
  return 0;
}
...
int main() {
  try {
    ...
    std::cerr << "cost example failed: " << error.message << "\n";
    return 1;
  }
}
